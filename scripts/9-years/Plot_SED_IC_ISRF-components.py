...
########################################################################################################################## Fit spectra
    ...
    if fit_IC:
        ...
        EdNdE_e = N_0 * E_e**(-gamma) * np.exp(-E_e * Ecut_inv)
        irf_components = [EdNdE_CMB, EdNdE_irf_IR, EdNdE_irf_starlight, EdNdE_irf]
        lss = ["--", "-.", ":", "-"]
        labels = ["CMB", "IR", "starlight", "total ISRF"]
        colors = ["darkorange", "red", "cadetblue", "blue"]
        lws = [1.8, 1.8, 1.8, 1.6]
...
if fit_pi0:
        ...
        if plot_till_100TeV:
            ...
            pyplot.errorbar(E_gamma, flux_pi0, label = r"$\pi^0$", color = "black", ls = '-')
        else:
