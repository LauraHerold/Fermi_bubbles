""" Plots the SED in a given latitude stripe, fits powerlaw, IC and pi0 spectra. """
...
latitude = 0 #[-55.0, -45.0, -35.0, -25.0, -15.0, -8.0, -4.0, 0.0, 4.0, 8.0, 15.0, 25.0, 35.0, 45.0, 55.0]
...
dct  = dio.loaddict('dct/dct_' + input_data + '_' + data_class + '.yaml')
...
def powerlaw(pars): # powerlaw a * x^b with parameters a,b
    return lambda x: pars[0] * x**pars[1]

def plaw_cut(pars): # powerlaw a * x^b with parameters a,b
    return lambda x: pars[0] * x**pars[1] * np.exp(-x/pars[2])



########################################################################################################################## Load ISRF data for IC spectra
...
########################################################################################################################## Fit powerlaw
    ...
    if fit_plaw:
        print " "
        print "xxxxxxxxxxxxxxxxxxxxxxxxxxxx      Powerlaw         xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        print " "
        if cutoff:
            ...
            m = Minuit(chi2, N_zero = N_zero, error_N_zero = 0.00001, Gamma = -0.3., error_Gamma = 0.1, E_cut = 1.e11, limit_E_cut = (0,1.e20), error_E_cut = 10., errordef = 1.)
            m.migrad()
            ...
            label = r'$\mathrm{PL}:\ n = %.2f$' %(-2-Gamma) + r'$\frac{\chi^2}{\mathrm{d.o.f.}} = %.1f$' %(chi2_value / dof)
            dof = binmax - binmin - 2
...
########################################################################################################################## Fit IC and/or pi0
    ...
    if fit_IC:
        print " "
        print "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx           IC            xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        print " "
        if cutoff:
    ...
    if fit_pi0:
        print " "
        print "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx           pi0            xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        print " "
