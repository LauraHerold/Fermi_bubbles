...
########################################################################################################################## Fit spectra
    ...
    if fit_plaw:
        ...
        N_0, gamma, Ecut_inv = 4.e6, 2.0, 0.
        fit = likelihood(plaw, background_map, total_data_map)                                          # Fit model = (lowE * k + c) to highE
        ...
        label = r'$\mathrm{PL}:\ \gamma = %.2f, $' %gamma + r'$-\log L = %.2f$' %TS
            
        if cutoff:
            fit = likelihood_cutoff(plaw, background_map, total_data_map)                                                          # Fit model = (lowE * k + c) to highE
            ...
            if Ecut_inv == 0:
                label = r'$\mathrm{PL}:\ \gamma = %.2f,$ ' %(gamma) + r'$E_{\mathrm{cut}} = \infty$ ' + ',\n' + r'$-\log L = %.2f$' %TS
            else:
                label = r'$\mathrm{PL}:\ \gamma = %.2f,$ ' %(gamma) + r'$E_{\mathrm{cut}} = %.1e\ \mathrm{GeV}$ ' %(1./Ecut_inv) + ',\n' + r'$-\log L = %.2f$' %TS

        flux_plaw = [(plaw(N_0, gamma, Ecut_inv)(E) * Es[E]**2 / dOmega[b][l] / deltaE[E] / expo_map[E]) for E in range(fitmin,fitmax)]
