...
IRFmap_fn = '../../data/ISRF_flux/Standard_8.5_0_0_Flux.fits.gz'   # Model for the ISRF
...
########################################################################################################################## Fit spectra
    ...
    if fit_plaw:
        ...
        label = r'$\mathrm{PL}:\ \gamma = %.2f$' %(gamma+2) #+ r', $-\log L = %.2f$' %TS_nocut
...
###################################################################################################################### Print total energy output
    ...
    if print_total_energy_output:
        for lower_bound_particle_energy in [1.]: #GeV
            ...
            print r"$$\rightarrow E_{\tot,\el} = \SI{", (E_tot_e/erg2GeV * V_ROI),"}{erg}.$$"
            print r"$$\left(\frac{\de E_\tot}{\de V}\right)_\pr = \SI{", (E_tot_p * 1.e12),"}{meV/cm^3} = \SI{", (E_tot_p/erg2GeV),"}{erg/cm^3},$$"
            print r"$$\rightarrow E_{\tot,\pr} =  \SI{", (E_tot_p * 1.e12 * V_ROI),"}{erg}.$$"
