...
for b in xrange(nB):
    ...
    for input in input_data:    
        ...
        for l in xrange(nL):
            map[l]  = np.asarray(diff_profiles[b][l])
            std_map[l] = np.asarray(std_profiles[b][l])

            
########################################################################################################################## plot energy flux with error bars


        difference = np.absolute(map[0] - map[1])
        total_std = (np.sqrt(std_map[0]) + np.sqrt(std_map[1]))**2
        label = input
        pyplot.errorbar(Es, difference, total_std, color=colours[colour_index], marker=markers[marker_index], markersize=markersize[markersize_index], markeredgewidth=0.4, linestyle=linestyle[linestyle_index], linewidth=0.1, label=label)
        colour_index += 1
        linestyle_index += 1
        marker_index += 1
        markersize_index += 1                
...
########################################################################################################################## cosmetics, safe plot
    ...
    pyplot.axis('tight')
    name = 'SED_difference_' +'_' + str(int(Bc[b]))
    fn = plot_dir + name + fn_ending
    pyplot.xscale('log')
    pyplot.yscale('log')

    if dB[b] == 10:
        pyplot.ylim((1.e-8,1.e-5))
    else:
