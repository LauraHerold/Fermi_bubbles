...
as well as the uncertainties in the absorption of CO emission along the line of sight to the GC (which is used as a tracer of molecular hydrogen)
...
Propagation of CR in the Galaxy can be computed with CR propagation tools, such as GALPROP \citep{2007ARNPS..57..285S}.
The maps of gamma-ray emission from interactions of CR with gas in different Galactocentric rings and from interaction
of CR electrons and positrons with ISRF can be used as templates for the corresponding components
...
The notion of the foreground and background emission may not be well defined, if we search for
...
of $\sim E^{-2.7}$ for energies from a few GeV to about PeV and $\sim E^{-3.3}$ for the CR electrons from GeV to TeV energies.
...
One of the caveats of this analysis is that the stationary gamma-ray emission in the ``on'' region can be larger than the emission in the ``off'' region.
But in any case the difference of the fluxes would have a soft spectrum characteristic for the propagated CR,
...
In any case, the ``on-off'' technic is a useful preliminary check in a search for populations of freshly accelerated CR.
The harder spectrum of the difference is a necessary condition for the presence of a population of CR with a hard spectrum
(after adjusting the zero-level in cases when the  ``off''  region is brighter than the ``on'' region),
...
First, since there is a tentative asymmetry in diffuse gamma-ray emission near the GC at high energies with a spectrum
harder to the east of the GC relative to the emission to the west of the GC,
we estimate the amount of asymmetry by taking the difference in the gamma-ray data to the east and to the west of the GC 
after masking bright point sources.
Second, since the spectrum of the FBs at high latitudes as well as the tentative spectrum at low latitudes
...
The residual emission is used to estimate the emission from the components of emission with spectra harder than the typical 
...
With the many free parameters, this model absorbs as much of the gamma-ray emission at the base of the FBs as it can.
As a result, the residual emission at the base of the FBs in this model is smaller than the residuals in the other models considered in the paper.
