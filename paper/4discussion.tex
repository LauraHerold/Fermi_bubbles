...
in each latitude stripe. The local ``index'' of the spectrum at energy $E$ is
 \be 
n := \frac{\de \ln f}{\de \ln E} = -\alpha - 2 \beta \ln(\frac{E}{\SI{1}{GeV}})).
 \ee
In Figure \ref{fig:logpar_index} we show a comparison of this log-parabola index as a function of latitude for the different bubbles spectra at $E = \SI{500}{GeV}$. \Laura{You called it bubble! ;)} The SED index corresponds to $(2 - n)$.
...
\begin{figure*}[h!]
    ...
    \end{subfigure}
  	\caption{SED of rectangles-model residual in the latitude stripes $(\ang{-2}, \ang{2})$ (left) and $(\ang{-6}, \ang{-2})$ (right) for negative (blue) and positive longitudes (red). We determine the spectral index of a powerlaw (PL), of an electron distribution emitting gamma-rays via IC and of a proton distribution emitting gamma-rays via $\pi^0$-decay. \Laura{Should we add the spectrum for the latitude band $(\ang{2}, \ang{6})$ also, or just say that it looks similar?}}
