...
In order to quantitatively estimate the difference in the intensity of emission of the FB at high and at low latitudes, as well as the asymmetry
of the gamma-ray emission at the base of the FB,
we plot the latitude profiles to the left and to the right of the GC.
In Figure \ref{fig:Profiles} we show the profiles of SED integrated in the energy range $10 - \SI{100}{GeV}$ as a function 
...
and for the GALPROP model --  the sum of residual emission, the FB template, 
...
For negative longitudes, there is an increase of flux in the Galactic plane by at least a factor of 2 to 6 for all models relative to the gamma-ray emission from the FB at high latitudes. 
...
\includegraphics[width=0.5\textwidth]{plots/Profiles_l=0_source_range_1.pdf}
  	\caption{Latitude profiles of the residuals in 
	different model of the foreground emission and the data excluding the PS mask in 
	the units of SED integrated between 10 and 100 GeV to the east (left) and to the west (right) of the GC.}
  	\label{fig:Profiles}
\end{figure*}
...
\includegraphics[width=0.5\textwidth]{plots/SED_all_models_source_l=-5_b=0.pdf}
  	\caption{SED of the residuals, the data, and the difference in the data west minus east of the GC (excluding pixels in the PS mask). 
...
\begin{table*}
  \begin{center}
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|c|} % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
		...
		& $(\ang{-10}, \ang{0})$ & 2.0  & 2.2 & -- & 0.0 & 308 & {308}  \\ 
 		\hline
  		$(\ang{-2}, \ang{2})$ & $(\ang{0}, \ang{10})$  & 3.5  & 2.3 & -- & 0.0 & {210} & 2.4  \\ 
		& $(\ang{-10}, \ang{0})$  & 6.4  & 2.1 & -- & 0.0 & \Laura{350} & \Laura{350}   \\ 
 		\hline
  		$(\ang{-6}, \ang{-2})$ & $(\ang{0}, \ang{10})$  & 1.8  & 2.1 & 260 & 3.8 & 120 & 11  \\ 
		& $(\ang{-10}, \ang{0})$ & 2.8  & 2.2 & -- & 0.0 & {330} & {330} \\ 
 \hline
    \end{tabular}
  \end{center}
\end{table*}
...
we fit the maximal and minimal points in the envelope above 3 GeV with a power-law and a cutoff function.
...
\begin{table*}
  \begin{center}
    ...
    The spectra are normalized at $E_0 = 1$ GeV. \Laura{New values in parenthesis.}}
    \label{tab:summary}
    \begin{tabular}{| l |c|c|c|c|c|c|c|} % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
		 ...
		 {\hspace{2cm}Model} & Type  & norm & index & cutoff & cutoff 95\% stat \\ 
		       &        &   &  & {\rm TeV} & {\rm TeV}\\ 
		\hline
  		\multirow{2}{*}{Parametric, $\frac{dN_\g}{dE} = \left[{\rm \frac{1}{GeV\, cm^{2}\, s}}\right]$} & max & $\SI{8.0e-6}{}$  & 2.10 &  -- & 3.9 \\ 
		& min & $\SI{1.4e-6}{}$ & 2.04 &  0.26 & 0.14  \\ 
 		\hline
  		\multirow{2}{*}{IC, $\frac{d\Sigma_e}{dE} = \left[{\rm \frac{1}{GeV\, cm^2}}\right]$}& max & $\SI{1.8e10}{}$ \Laura{$\SI{4.0e10}{}$} & 2.71 &  -- & 59 \\ 
		& min & $\SI{2.5e9}{}$ \Laura{$\SI{5.6e9}{}$} & 2.60 &  1.1 & 0.43  \\ 
 		\hline
  		\multirow{2}{*}{Hadronic, $\frac{d\Sigma_p}{dqc} = \left[{\rm \frac{1}{GeV\, cm^2}}\right]$} & max & $\SI{4.4e11}{}$  \Laura{$\SI{1.0e12}{}$}& 2.15 &  -- & 276 \\ 
		& min & $\SI{4.9e10}{}$  \Laura{$\SI{1.1e11}{}$}& 2.00 &  2.3 & 1.1  \\ 
 \hline
    \end{tabular}
  \end{center}
\end{table*}
