...
\includegraphics[width=0.5\textwidth]{plots/SED_all_models_source_l=-5_b=0.pdf}
  	\caption{SED of the model residuals, the data with PS mask and the difference in the data with PS mask West minus East. The width of the regions in longitude is $\ang{10}$, i.e. $\ang{0}$ to $\ang{10}$ to the East of the GC (left) and $\ang{-10}$ to $\ang{0}$ to the West of the GC (right). \dima{I don't think it makes sense to keep the difference in the data on the left plot, since there are basically no points. Can we remove it there?}}
  	\label{fig:SED_all}
\end{figure*}
